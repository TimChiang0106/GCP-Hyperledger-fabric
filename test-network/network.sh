...
# default
#export PATH=${ROOTDIR}/../bin:/root/fabric-samples/bin:$PATH
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    ${CONTAINER_CLI} volume rm docker_orderer.cloudmile.com docker_peer0.org1.cloudmile.com docker_peer0.org2.cloudmile.com
    ...
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf channel-artifacts logs-deploy-chaincode.txt *.tar.gz'
  fi
}
